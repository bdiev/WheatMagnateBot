...
function compactDetails(details) {
  ...
  for (const [key, value] of Object.entries(details)) {
    if (value == null || ['string', 'number', 'boolean'].includes(typeof value)) compact[key] = typeof value === 'string' ? value.slice(0, 512) : value;
...
async function recordOperationalEvent(pool, event = {}) {
  ...
  try {
    ...
    `, [
      ...
      String(event.source || 'system').slice(0, 64), String(event.title || event.eventType || 'System event').slice(0, 255),
      compactDetails(event.details), event.actor ? String(event.actor).slice(0, 128) : null,
      event.resourceKey ? String(event.resourceKey).slice(0, 255) : null, correlationId,
      event.sourceRecordType ? String(event.sourceRecordType).slice(0, 64) : null,
      event.sourceRecordId == null ? null : String(event.sourceRecordId).slice(0, 128), Boolean(event.sensitive), event.occurredAt || null
    ]);
    return result.rows[0] || null;
  } catch (err) {
    if (err?.code !== '42P01' && err?.code !== '42703') console.error('[OperationalEvents] Failed to record event:', err.message);
    return null;
  }
}

module.exports = { compactDetails, eventTypeFromLog, newCorrelationId, recordOperationalEvent, severityFromLevel };
