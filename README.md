...
The shared Minecraft chat pipeline sends messages sequentially and keeps only a bounded, short-lived queue. By default, a player may burst up to 8 distinct messages per 10 seconds, while repeated copies of the same normalized message are suppressed for 5 seconds. The website archive/live feed and Discord receive the same accepted messages plus one flood summary; suppressed copies are not written to the archive. Queue limits and timing can be adjusted with the `DISCORD_CHAT_*` variables in `.env.example`.
