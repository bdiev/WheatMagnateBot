...
assert.match(appSource, /profile\.toneConfidence/, 'style rows must show tone confidence rather than a bare label');
assert.match(appSource, /languageBreakdown[\s\S]*profile\.languageConfidence/, 'style rows must show primary/secondary language confidence');
assert.match(appSource, /data-confidence="\$\{escapeHtml\(learningStatus\)\}"/, 'style rows must expose evidence quality');
assert.match(appSource, /inquisitive, playful, direct, formal/, 'administrator tone overrides must include the richer classifier categories');
...
assert.doesNotMatch(stylesSource, /\.child-ai-style-row\s*\{[^}]*content-visibility:/s, 'mobile style rows must remain painted to prevent avatar flicker in Safari');
assert.match(stylesSource, /\.child-ai-style-confidence\[data-confidence="strong"\]/, 'strong evidence must have a distinct confidence badge');
