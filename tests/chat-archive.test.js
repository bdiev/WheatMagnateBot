...
assert.match(indexSource, /styles\.css\?v=153/, 'the updated mobile layout must use a fresh stylesheet URL');
assert.match(indexSource, /app\.js\?v=158/, 'the updated dashboard behavior must use a fresh script URL');
assert.match(serviceWorkerSource, /CACHE_VERSION = '136'/, 'the app shell cache must be replaced after dashboard behavior changes');
