...
assert.match(indexSource, /styles\.css\?v=226/, 'the updated mobile layout must use a fresh stylesheet URL');
assert.match(indexSource, /app\.js\?v=229/, 'the updated dashboard behavior must use a fresh script URL');
assert.match(serviceWorkerSource, /CACHE_VERSION = '229'/, 'the app shell cache must be replaced after dashboard behavior changes');
