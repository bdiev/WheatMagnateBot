...
assert.match(indexSource, /styles\.css\?v=150/, 'the updated mobile layout must use a fresh stylesheet URL');
assert.match(indexSource, /app\.js\?v=154/, 'the updated chat behavior must use a fresh script URL');
assert.match(serviceWorkerSource, /CACHE_VERSION = '131'/, 'the app shell cache must be replaced after the UI update');
