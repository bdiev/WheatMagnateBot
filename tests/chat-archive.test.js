...
assert.match(indexSource, /styles\.css\?v=148/, 'the updated mobile layout must use a fresh stylesheet URL');
assert.match(indexSource, /app\.js\?v=152/, 'the updated chat behavior must use a fresh script URL');
assert.match(serviceWorkerSource, /CACHE_VERSION = '129'/, 'the app shell cache must be replaced after the UI update');
